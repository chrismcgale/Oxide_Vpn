...
- **`mimicry`** — **stealth tier 2 (protocol mimicry)** codec: builds a realistic TLS 1.3 handshake (ClientHello w/ SNI + cipher suites + key_share, ServerHello) and frames datagrams as TLS `application_data` records. `wg-core::MimicTransport` speaks it over TCP (client single-stream; server accept + demux by TCP source addr), obfuscating payloads first — so the flow looks like an HTTPS session (blocking it = blocking HTTPS). Pure codec + a transport variant on `Transport::Mimic`. It's mimicry, not real TLS (endpoints don't validate); security is the WG AEAD inside.
