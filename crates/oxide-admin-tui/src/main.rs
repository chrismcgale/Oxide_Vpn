...
use app::{adoption, human_bytes, load_fraction, App, CostModel, Tab};
...
/// One row per server: location, load bar, feature badges, bandwidth, health, cost.
fn draw_servers(f: &mut Frame, app: &App, area: Rect) {
    ...
    let list = List::new(items)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Servers — id · location · load · features · ↑tx ↓rx · health · $ "),
        )
        .highlight_style(
            Style::default()
                .bg(Color::Blue)
...
fn draw_footer(f: &mut Frame, app: &App, area: Rect) {
    let help = "1/2/3 or ←/→ tabs · ↑/↓ select · r refresh · q quit";
    f.render_widget(
        Paragraph::new(vec![
            Line::from(Span::styled(
                format!("  {help}"),
